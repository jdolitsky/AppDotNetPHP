 ...
 * done for you, use the included AppDotNet class, which makes does much
 * less automatically.
 */
class EZAppDotNet extends AppDotNet {

	public function __construct($clientId=null,$clientSecret=null,$redirectUri=null) {
		global $app_clientId,$app_clientSecret,$app_redirectUri;
	...
	public function getAuthUrl($scope=null) {
		...
		// use the scope from the settings.php file if it's there
		if (!$scope && isset($app_scope)) {
			$scope = $app_scope;
		}

		// call the parent with the scope we've decided on
		return parent::getAuthUrl($scope);
	}
